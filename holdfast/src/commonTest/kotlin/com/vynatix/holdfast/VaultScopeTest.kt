...
/**
 * Issue 01 — Vault.scope + Vault.Companion.defaultScope settable-once.
 *
 * `Vault.defaultScope` is a process-singleton settable at most once per process. Tests
...
class VaultScopeTest {
    private class TestVault : Vault<TestVault>()

    @Test
    fun vault_scope_defaults_to_Vault_Companion_defaultScope() {
        val vault = TestVault()
        assertSame(Vault.defaultScope, vault.scope)
    }
