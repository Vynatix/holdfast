...
class EventfulVaultTest {
    ...
    @Test fun nestedActionEmitsFireInOrderOnOuterCommit() =
        runBlocking {
            val v = FixtureEventfulVault()
            val received = mutableListOf<FixtureEvent>()
            val collectJob =
    ...
    @Test fun nestedActionInnerRollbackKeepsOuterEvents() =
        runBlocking {
    ...
    @Test fun customDataEventsRoundTrip() =
        runBlocking {
            val v = FixtureEventfulVault()
            val received = mutableListOf<FixtureEvent>()
            val collectJob =
            ...
            withTimeoutOrNull(2_000) { collectJob.join() }
            val expected: List<FixtureEvent> =
                listOf(
