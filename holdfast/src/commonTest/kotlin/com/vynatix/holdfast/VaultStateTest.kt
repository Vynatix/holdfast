...
private class StateTestVault : Vault<StateTestVault>() {
    val count by state { 0 }
    val label by state { "initial" }
}

private class StateOwnerA : Vault<StateOwnerA>() {
    val a by state { 0 }
}

private class StateOwnerB : Vault<StateOwnerB>() {
    val b by state { 0 }
}

private class MixedTypeStateVault : Vault<MixedTypeStateVault>() {
    val intState by state { 1 }
    val stringState by state { "two" }
    val listState by state { listOf(3, 4) }
}

private class TwoZeroVault : Vault<TwoZeroVault>() {
    val a by state { 0 }
    val b by state { 0 }
}
