...
| **Time as an input (#20 PR 2, R10).** Getter-based `Store.clock` plus a per-instance `bindClock(Clock?)`, mirroring `scope`/`bindToScope`; bound clocks restored at `storeTest` teardown. **Source break**, inside this window: a subclass property named `clock` now conflicts. | **Done** (S) |
...
- Eager state registration via `provideDelegate` — kills the snapshot/restore-on-untouched-store surprise class. Lands as #20 PR 4 (R5): a declaration registry (declared vs materialized states, initializers kept out of `propertiesLock`, cycles and initializer writes fail fast) plus consistent single-store snapshots (M/L) — **Done** (#20 PR 4): `StateDelegate.provideDelegate` declares every state at construction; initializers run once behind per-state latches with cycle detection, read committed values only, and may not write; a second declaration of a name fails fast unless the same declaration site runs again; `snapshot()` materializes never-read states and takes a write-bracketed consistent cut; `derived` backing states leave `stateNames` and restore only into their own store. iOS (`StoreThreadLocals.ios.kt`) is unverified until a macOS run
- #20 PR 3: writes into an already-applied transaction fail loudly — `mutate`, `emit` and nested `action` from an observer into its own committing store surface instead of being silently lost. Lands with or after the observer-exception default below (S) — **Done**, together with that default. A refused nested `action`/`atomic` returns an `Error` the observer must check; it is not reported. A blocking `action`/`atomic` from inside a `suspendAction`/`suspendAtomic` commit (observers, bridge publishes, inline event collectors, frame observers, a later participant targeting an earlier one) now returns `Error` instead of spinning. Still open: such a call on a later `suspendAtomic` participant that has not committed yet (closed by PR 11), and on iOS/wasmJs from a nested `withContext(dispatcher)` inside the commit; and another thread's bare `mutate` under a suspending commit is refused rather than waiting until the fail-fast guard's "body is running" marker exists
- #20 PR 5 (R4): experimental `reset()` from retained initializers — stages initializer output raw, only where it differs, as one transaction (S/M) — **Done** (#20 PR 5): `reset()` materializes every declared state before its transaction opens, then re-runs the initializers in fresh-store order (a read of another declared state resolves that state's reset first, so forward references see reset values; other stores and `derived` states read committed), stages the output raw and only where it differs from what the transaction holds, as one action (a savepoint inside an action, part of the frame inside `atomic`); a throwing initializer or a cycle rolls it all back. Until its transaction ends, `removeState`/`clearStates` refuse every state it has resolved (and they now check the whole savepoint chain for pending writes). Known limit: an initializer reading a `derived` of states the reset changes reads its pre-reset value. Internal `stageResetOfDeclaredStates(txn)` resets several stores into one frame's roots, one transaction per store (the #21 `reset(node)` shape), with extension points for keyed families (PR 12) and attachments (PR 10). iOS unverified until a macOS run
- `suspendAction` disposed-store check matches blocking `action`; `emit()` gains the ownership check `mutate` has (S)
- Observer-exception default: log loudly instead of silent swallow; document `uncaughtObserverHandler` where users will find it (S) — **Done** (#20 PR 3): with no handler, post-commit failures go to standard error (JVM/Android) or standard output (iOS/wasmJs); documented in the `effect` KDoc and GUIDE §4.4, in the pitfalls table and in the module README
...
- #20 PR 6 (R1): codec-bearing states, `snapshot().encode()`/`StoreSnapshot.decode`, `RestorePolicy`, typed snapshot reads; stable `StateCodec` (see principle 6), the rest experimental (M/L) — **Done** (#20 PR 6): `bridge.Codec` extends the stable `StateCodec`; an experimental `state(…, codec)` overload (codec-less calls still resolve to the stable one); canonical v1 text through a hand-rolled, depth-capped JSON reader and writer; `restore(snapshot, policy)` plans outside the store's locks, then stages raw in one action, with a type witness and a `RestoreReport`; the frozen `restore` now ignores unknown names; snapshots gain value equality and typed reads (`snapshot[state]`) that work after dispose; no exception carries a state value. GUIDE §16.2 with a compiled `KSerializerCodec` recipe. iOS unverified until a macOS run
- #20 PR 7 (R2): opt-in `SchemaVersioned` schema version and `migrate` upcasting (S) — **Done** (#20 PR 7): a `Store` subclass opts in by implementing `SchemaVersioned` (no new `Store` member); snapshots record the store's version and `encode()` writes it; `restore` checks it first under every policy — an older decoded snapshot is upcast by `migrate` on a copy of its text (`EncodedSnapshotView`: per-state codec text, plus a read-only families section for R7), a newer or captured-other-version snapshot or a throwing `migrate` fails with `SnapshotMigrationException` and changes nothing; `migrate` runs in the planning phase inside the initializers' no-write region. GUIDE §16.3. iOS unverified until a macOS run
- #20 PR 8 (R3): state tags (`Secret`/`UserAuthored`/`Remote`), snapshot scopes, sterile restore, and Secret redaction across snapshots, middleware and the test harness (M) — **Done** (#20 PR 8): experimental `StateTag` and `SnapshotScope` (closed, not exhaustive; `Secret`+`UserAuthored` and `UserAuthored`+`Remote` refused at declaration), `state(…, codec, tags)`, the one lookup `State.tags`/`taggedStates` (a `derived` with a Secret source is Secret); Secret values stay in memory (undo stays lossless) and are withheld at read time — `Redacted` from typed reads outside a `Raw` capture, `null` in `encode()`, `<redacted>` in `render()`, `Redacted` in `:holdfast-testing` timelines and bridge views, value matchers refused, never in built-in middleware output, withheld from `:holdfast-hallmark` validation errors of `boxed(…, tags)` states, from `suspendValidateAndMutate`'s and from `shouldBeBoxedAs` failures; `snapshot(UserAuthored)` captures exactly the tagged states; `encode(includeRemote)` leaves Remote states out by default; `restore(…, sterile = true)` resets Remote states to their initial values in the restore's transaction, computing them (and any state the restore brings to life) from the restored values. GUIDE §16.4 answers open question 1 (a tag, not a `RedactingTransformer`). iOS unverified until a macOS run
- #20 PR 9 (R6): experimental `derivedState(...)` and `merged(local, remote)` returning `DerivedState<T>`; the legacy `derived` `Pair` stays until the 0.7.0 triage (S/M) — **Done** (#20 PR 9): a sealed, read-only `DerivedState<T>` (declared with `by`, its own `Disposable`) over an unregistered `distinct` backing state that snapshots, `restore`, `reset()`, `encode()`, `properties` and `taggedStates` never see; it recomputes once per source commit — for sources on other stores too, queued on the committing store and run after that commit releases it (one recompute for a two-store `atomic` frame) — through `derived`'s never-blocking hand-off; `merged` takes two different declared states of its store (their tags are the caller's to choose), carries neither tag, and a Secret source taints any derived state. Every observation path (`effect`, flows, `collectAsState`, derived sources, the test harness) resolves through `@StoreInternalApi observableBacking()`. A derived state subscribes before its initial compute (a racing source commit is recomputed, not lost), and a recompute reads committed values only (its compute may not write): nested in a blocking action on a source's store, it waits for that action to end, while a `suspendAction` on one, even parked on its thread, never holds it back. Until PR 11, a recompute racing another thread's frame can still commit a torn pair, and an initial compute reading a non-source state of a store its thread holds reads that action's uncommitted writes. iOS unverified until a macOS run
- #20 PR 10: internal `StoreAttachment` slot (dispose/reset notifications), which unblocks #21 independently of hydration (S) — **Done** (#20 PR 10): `@StoreInternalApi` `StoreAttachment` (`onStoreReset`, `onStoreDisposed`, `persistenceKeys`, all defaulted) addressed by identity-compared `StoreAttachmentKey`s; `internalAttachIfAbsent` has one winner under races and runs `create` once per successful attach, under the slot's own lock (ordered after `transactionLock`, `middlewareLock` and initializer latches, before `propertiesLock`, so `create` may only build, register internal states and attach under other keys); `internalAttachment`/`internalAttachments` (attach order) read lock-free and answer empty after dispose. The slot is independent of the state registry, so a base class's `init` can attach before subclass states are declared (#21 T3) and the attachment resolves declarations lazily. `reset()` tells attachments inside its transaction, outside the no-write region, so their writes commit or roll back with it (a throwing one rolls the reset back; a sterile `restore` tells none); `dispose()` closes the slot under `transactionLock` and tells each attachment once, last, holding no lock it took, each failure reported (under a dispose log line of its own when no handler is set) and isolated — an attach racing it is either told or refused. iOS unverified until a macOS run
...
- #20 PR 11 (R9): frames apply every participant before any fans out, derivations settle once per outermost entry or frame (`SettleScope`), and a consistent multi-store cut (L) — **Done** (#20 PR 11): `atomic`/`suspendAtomic` apply every participant inside one write bracket, then fan each out in lock order (observers → bridge publish → events; `FrameObserver.onFrameCommitted` last), so a consistent read sees a frame whole or not at all (an outermost frame, or one sharing no store with its enclosing action or frame: a shared store joins as a savepoint and applies with the enclosing transaction — plan amendment (d)), a failed apply or fanout (a publish's `CancellationException` included) still fans out every applied participant and rolls back the rest, savepoint participants included, with `onTransactionError` only on those, an observer of one participant finds the others applied — and may not write into any of them (BREAKING: a write into a later participant used to join the frame), and the `suspendAtomic` gap where a blocking call on a later participant waited forever is closed; every `action`/`atomic`/`suspendAction`/`suspendAtomic` (and the internal recompute attempt, the harness's open transactions) opens or joins a per-thread settle scope, carried across coroutine dispatch, and `derivedState`/`merged` recompute once when the outermost one has released every store, lowest rank first, each reading its sources from one committed cut (no torn pair; the initial compute too, catching up when it read an uncommitted value) — answering #20's open question 2: coalescing is not a mode; frames' post-commit drains move to that settle (BREAKING), and a settle caps the runs of each task and each store drain, so no feedback loop keeps it going, and reports the cut through the store's `uncaughtObserverHandler`; an outermost `suspendAction`/`suspendAtomic` refuses an already-cancelled caller on every platform and returns its committed result to a caller cancelled after the body; the legacy `derived` keeps per-fire recompute until 0.7.0; internal `captureConsistent(stores)` is #21's T4 cut. The warm 10k-action loop on the JVM measures within noise of the tree before it. iOS unverified until a macOS run (its shared sources type-check on Linux)
- #20 PR 12 (R7): keyed state families — `val docs by keyedState<K, T> { key -> … }` — with per-key eviction and snapshot support (M) — **Done** (#20 PR 12): an experimental `keyedState<K, T>(transformer, distinct, codec, keyCodec, tags) { key -> … }` family declared under its property's name (families and states share names), whose entries are ordinary states kept outside the declared-state registry, created at a key's first `docs[key]` through the declared-state latch and no-write region, and the same `State` while they live. `evict`/`evictAll` are transactional — staged like writes, the last operation on an entry winning across savepoints, applied inside the commit's write bracket, rolled back with it — and leave a stale handle whose writes throw, shutting down only the evicted entry's observers, bridge and `observeFrom` subscriptions; an eviction from the store's own commit fanout defers through the post-commit queue (the one D16 exception), evicting the entries live at the call in a transaction of its own that never waits for the store (a never-blocking attempt handed to the store's holder when busy, so a suspending call's drain cannot spin on the mutex it just handed on); inside a suspending body, or from an `atomic` body that does not enroll the store, a `get` leaves a staged eviction staged (a write cancels it). Snapshots capture every live entry in their consistent cut (listing again when an entry of a captured family came to life while a captured store committed or took an inbound bridge write, and after a few such retries holding entry creation back for the listing and cut, so a capture always completes and never blocks a writer), `keysOf` and typed reads work for captured and decoded snapshots, `encode()` writes a family as `{encodedKey: text|null}` under its name (a family without a codec and a keyCodec is skipped), `restore` creates entries and never evicts, `reset()`/sterile restore re-stage live entries (and those they bring to life, never one their transaction evicts), `migrate` edits families, Secret families withhold values (not keys), and no message or middleware sample names a key (`family[*]`). `@StoreInternalApi` `internalKeyedAddress`, `internalKeyedFamily` and a membership listener (each entry's added strictly before its evicted) serve #21; `:holdfast-testing`'s `shouldMatchSnapshotOf` compares families entry by entry. Known gap kept: another thread's eviction under a `suspendAction` joins its transaction. iOS unverified until a macOS run
