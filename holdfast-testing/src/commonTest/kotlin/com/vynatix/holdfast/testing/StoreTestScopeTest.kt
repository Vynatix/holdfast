...
class VaultTestScopeTest {

    @Test
    fun bodyExecutes() = vaultTest {
        var ran = false
        run { ran = true }
        assertTrue(ran)
    }
    ...
    @Test
    fun trackIsIdempotentByIdentity() = vaultTest {
        val v = TinyVault()
        val h1 = track(v)
        val h2 = track(v)
        assertSame(h1, h2)
    ...
    @Test
    fun distinctVaultsGetDistinctHandles() = vaultTest {
        val a = TinyVault()
        val b = TinyVault()
        val ha = track(a)
        val hb = track(b)
