...
/**
 * Selects what a [HoldfastHandle] records about its tracked vault during a test.
 *
 * The chosen mode is preserved on the handle and drives the privileged
 * recorder middleware installed by [HoldfastTestScope.track]:
 *
 *  - [All] grows the [HoldfastHandle.timeline] unbounded for the test's lifetime.
 *  - [None] does not install the recorder at all, so no events are captured
 *    and there is no per-action overhead. [HoldfastHandle.timeline] returns an
