# Changelog — `:vault-compose`

All notable changes to `:vault-compose` are documented here. The format is
...
- Version bump only — the `:vault-compose` public surface is unchanged in
  2.0. `@Composable State<T>.collectAsState()` and
  `@Composable rememberDisposable { ... }` continue to work identically.

### Removed

- Nothing removed from the `:vault-compose` public surface.

### Changed (behavior, signature stable)

- `:vault-compose` re-exports vault state through Compose's `State<T>`. With
  `:vault-coroutines` 2.0's lossless-conflated `asFlow()` semantics
...
- `:vault-compose` 2.0 stays Android + iOS pending Compose Multiplatform
  ...
  not on the `:vault-compose` roadmap; they are deferred at the
  `:vault` core level (Threading semantics).
