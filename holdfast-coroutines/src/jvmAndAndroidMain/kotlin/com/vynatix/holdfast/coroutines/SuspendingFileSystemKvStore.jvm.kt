...
actual class SuspendingFileSystemKvStore actual constructor(directory: String) : SuspendingKvStore {
    ...
    actual override suspend fun get(key: String): String? = withContext(Dispatchers.IO) {
    ...
    actual override suspend fun snapshot(): Map<String, String> = withContext(Dispatchers.IO) {
        val files = root.listFiles { f -> f.isFile && !f.name.startsWith(TMP_PREFIX) }
