...
private class RecordingMiddleware(
    ...
    override fun onTransactionError(
        context: MiddlewareContext<MwVault>,
        error: Throwable,
    ) {
        log.add("$tag.error")
    }
...
class SuspendActionMiddlewareTest {
    @Test
    fun startedHookFiresOnSuspendAction() =
        runBlocking {
            val v = MwVault()
    ...
    @Test
    fun completedHookFiresOnSuccessfulSuspendAction() =
        runBlocking {
            val v = MwVault()
    ...
    @Test
    fun errorHookFiresOnThrowingSuspendAction() =
        runBlocking {
            val v = MwVault()
            val log = mutableListOf<String>()
            v.middlewares(RecordingMiddleware("M", log))
    ...
    @Test
    fun concentricRingOrderingAcrossTwoMiddlewaresOnError() =
        runBlocking {
            val v = MwVault()
            val log = mutableListOf<String>()
    ...
    @Test
    fun concentricRingOrderingAcrossTwoMiddlewaresOnSuccess() =
        runBlocking {
            val v = MwVault()
            val log = mutableListOf<String>()
            v.middlewares(
    ...
    @Test
    fun runCatchingIsolatesEachStartedHookFailureFromOtherMiddleware() =
        runBlocking {
            val v = MwVault()
            val log = mutableListOf<String>()
    ...
    @Test
    fun runCatchingIsolatesEachCompletedHookFailureFromOtherMiddleware() =
        runBlocking {
            val v = MwVault()
            val log = mutableListOf<String>()
            v.middlewares(
    ...
    @Test
    fun runCatchingIsolatesEachErrorHookFailureFromOtherMiddleware() =
        runBlocking {
            val v = MwVault()
            val log = mutableListOf<String>()
            v.middlewares(
    ...
    @Test
    fun middleware_ordering_is_identical_across_action_and_suspendAction() =
        runBlocking {
    ...
    @Test
    fun bodyMaySuspendBetweenStartedAndCompleted() =
        runBlocking {
            val v = MwVault()
            val log = mutableListOf<String>()
            v.middlewares(RecordingMiddleware("M", log))
