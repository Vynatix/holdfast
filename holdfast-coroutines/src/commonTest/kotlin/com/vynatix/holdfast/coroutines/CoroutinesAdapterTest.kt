...
private class FlowVault : Store<FlowVault>() {
    val n by state { 0 }
    val s by state { "init" }
}

class AsFlowTest {
    @Test fun asFlowEmitsCurrentValueImmediately() =
        runBlocking {
            val v = FlowVault()
    ...
    @Test fun asFlowDeliversCommittedValuesWithBreathingRoom() =
        runBlocking {
    ...
    @Test fun asFlowDisposesObserverOnCollectorCancel() =
        runBlocking {
            val v = FlowVault()
...
class AsStateFlowTest {
    @Test fun asStateFlowExposesCurrentAndSubsequentValues() =
        runBlocking {
            val v = FlowVault()
            v action { n mutate 5 }
...
class FirstAndAwaitValueTest {
    @Test fun firstSuspendsUntilPredicateHolds() =
        runBlocking {
            val v = FlowVault()
            val collector =
                async {
    ...
    @Test fun awaitValueSuspendsUntilEquality() =
        runBlocking {
            val v = FlowVault()
            val collector =
                async {
