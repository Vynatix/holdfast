...
private class EventingAccountVault(initial: Long = 0) : EventfulStore<EventingAccountVault, TransferEvent>() {
    val balance by state { initial }
}

class SuspendAtomicSuccessTest {

    @Test fun transferBetweenTwoVaultsCommitsBoth() = runBlocking {
    ...
    @Test fun suspendAtomicReturnsBodyValueViaSuccess() = runBlocking {
    ...
    @Test fun suspendAtomicWithDifferentVaultClasses() = runBlocking {
        val acct = AccountVault(initial = 1000)
        val ledger = LedgerVault()
        val r = suspendAtomic(acct, ledger) {
    ...
    @Test fun suspendAtomicBodyMaySuspendBetweenMutations() = runBlocking {
...
class SuspendAtomicRollbackTest {
    ...
    @Test fun bodyThrowRollsBackBoth() = runBlocking {
        val a = AccountVault(initial = 100)
        val b = AccountVault(initial = 0)
        val r = suspendAtomic(a, b) {
            a { balance update { it - 30 } }
    ...
    @Test fun bodyThrowDoesNotFireObservers() = runBlocking {
        val a = AccountVault(initial = 100)
        val b = AccountVault(initial = 0)
        val seenA = mutableListOf<Long>()
        val seenB = mutableListOf<Long>()
...
class SuspendAtomicCancellationTest {
    ...
    @Test fun bodyCancelledRollsBackBoth() = runBlocking {
        val a = AccountVault(initial = 100)
        val b = AccountVault(initial = 0)
...
class SuspendAtomicDeadlockPreventionTest {
    ...
    @Test fun deadlockPreventionViaLockOrderKey() = runBlocking {
        ...
        val workers = 4
        val perWorker = 25
        val jobs = List(workers) { workerIdx ->
            scope.async {
                repeat(perWorker) {
                    if (workerIdx % 2 == 0) {
                        suspendAtomic(a, b) {
...
class SuspendAtomicNestedTest {
    ...
    @Test fun nestedSuspendAtomicReusesParentLocksForOverlappingVaults() = runBlocking {
        val a = AccountVault(initial = 0)
        val b = AccountVault(initial = 0)
    ...
    @Test fun nestedSuspendAtomicOnSameVaultsIsTrivial() = runBlocking {
...
class SuspendAtomicEventsTest {
    ...
    @Test fun eventsFireAfterCommitInLockOrder() = runBlocking {
        ...
        val collectorA = scope.launch {
            a.events.collect { ev ->
                timeline += "a-event"
            }
        }
        val collectorB = scope.launch {
            b.events.collect { ev ->
                timeline += "b-event"
            }
        }
...
class SuspendAtomicMutualExclusionTest {
    ...
    @Test fun suspendAtomicSerializesWithSuspendActionOnSharedVault() = runBlocking {
        ...
        val actionJob = async(Dispatchers.Default) {
            a.suspendAction {
        ...
        gate.await()
        val atomicJob = async(Dispatchers.Default) {
            suspendAtomic(a, b) {
...
class SuspendAtomicRequireVaultTest {
    ...
    @Test fun emptyVaultsThrowsIllegalArgument() = runBlocking {
        try {
    ...
    @Test fun singleVaultSuspendAtomicWorksLikeMiniSuspendAction() = runBlocking {
        val a = AccountVault(initial = 0)
        val r = suspendAtomic(a) {
