...
- **wasmJs is single-threaded by assumption**: `currentThreadId()` returns `0` for everyone there — any new thread-identity logic must tolerate this. Expect/actual pairs live in `platform/Threading`, `platform/FrameLocal` (frame- and fanout-marker slots), `platform/UncaughtFailureLog` and `bridge/FileSystemKvStore`, plus `:holdfast-coroutines`' `FrameMarkerContext` (custom intermediate source sets `jvmAndAndroidMain`/`jvmAndAndroidHostTest` share JVM+Android actuals).
