...
/**
 * Issue 02 — `Vault.bindToScope(scope)` rebindable per-vault scope binding.
 *
...
class VaultBindToScopeTest {
    private class PlainVault : Vault<PlainVault>()

    /** Per-vault subclass override (resolution level 2) — must beat any bound scope. */
    private class OverrideVault(private val s: CoroutineScope) : Vault<OverrideVault>() {
    ...
    fun unbound_vault_returns_defaultScope() {
        val v = PlainVault()
        assertSame(Vault.defaultScope, v.scope)
    }
    ...
    fun bindToScope_makes_vault_scope_return_the_bound_scope() {
        ...
        assertNotSame(Vault.defaultScope, v.scope)
    }
