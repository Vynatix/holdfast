...
private class ParallelVault : Vault<ParallelVault>() {
...
private class ReentrancyVault : Vault<ReentrancyVault>() {
    val count by state { 0 }
    val flag by state { "init" }
}

private class StressVault : Vault<StressVault>() {
    val n by state { 0 }
    val s by state { "" }
    val list by state { listOf<Int>() }
}

private class IdentityCheckBridgeVault : Vault<IdentityCheckBridgeVault>() {
    val count by state { 0 }
}
...
class ParallelActionTest {
    ...
    @Test
    fun concurrentReadModifyWriteActionsHaveNoLostUpdates() = runBlocking {
        val v = ParallelVault()
        val workers = 8
    ...
    @Test
    fun concurrentMultiStateMutationsStayAtomicallyInLockstep() = runBlocking {
        val v = ParallelVault()
        val workers = 8
    ...
    @Test
    fun concurrentReadersObserveMonotonicValueUnderConcurrentWriters() = runBlocking {
        val v = ParallelVault()
        val readers = 4
        val writers = 4
        val opsPerWorker = 200
        val readJobs = List(readers) {
            async(Dispatchers.Default) {
                var lastSeen = 0
                repeat(opsPerWorker) {
    ...
    @Test
    fun failingActionInOneWorkerDoesNotCorruptStateForOtherWorkers() = runBlocking {
        val v = ParallelVault()
        val workers = 8
        val opsPerWorker = 50
    ...
    @Test
    fun activeTransactionIsClearedAfterEverySuccessfulAndFailedAction() = runBlocking {
        val v = ParallelVault()
    ...
    @Test
    fun crossThreadMutateOnSameStateSerializesThroughTransactionLock() = runBlocking {
        val v = ParallelVault()
        val n = 1000
        val jobs = List(2) {
            async(Dispatchers.Default) {
    ...
    @Test
    fun parallelFailingActionsDoNotInterleaveOrLeakActiveTransaction() = runBlocking {
        val v = ParallelVault()
        val workers = 8
        val attempts = 100
    ...
    @Test
    fun concurrentBridgeAttachAndDetachDoesNotCorruptObserverList() = runBlocking {
    ...
    @Test
    fun parallelActionsOnDifferentVaultsRunFullyInParallel() = runBlocking {
        val vaults = List(4) { ParallelVault() }
        val opsPerVault = 500

        val jobs = vaults.map { v ->
            async(Dispatchers.Default) {
                repeat(opsPerVault) {
                    v action { count mutate count.value + 1 }
                }
    ...
    @Test
    fun concurrentRemoveStateAndMutateBehavesPredictably() = runBlocking {
        val v = ParallelVault()
        val workers = 6
        val opsPerWorker = 100
...
class SameThreadReentrancyTest {
    ...
    fun recursiveActionFromInsideMiddlewareDoesNotDeadlock() {
        ...
        val v2 = ReentrancyVault()
        v1.middlewares(object : Middleware<ReentrancyVault>() {
...
class StressTest {
    ...
    @Test
    fun tenThousandActionsAcrossEightWorkersAllAccountedFor() = runBlocking {
        val v = StressVault()
        val workers = 8
    ...
    @Test
    fun oneHundredThousandActionsAcrossSixteenWorkersStayConsistent() = runBlocking {
        val v = StressVault()
        val workers = 16
    ...
    @Test
    fun randomMixOfMutationsRollbacksAndReadsLeavesStateConsistent() = runBlocking {
        val v = StressVault()
        val workers = 8
    ...
    @Test
    fun manyShortLivedSubscribersOnHighFrequencyMutationsDoNotLeakOrDeadlock() = runBlocking {
        val v = StressVault()
    ...
    @Test
    fun multiStateMultiVaultStressDoesNotInterleave() = runBlocking {
    ...
    @Test
    fun mixedMiddlewareTransformerEffectStressOnSingleVaultStaysConsistent() = runBlocking {
        val v = StressVault()
        val mwInvocations = atomic(0)
        v.middlewares(object : Middleware<StressVault>() {
