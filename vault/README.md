...
| `com.vynatix:vault-validation` | Boundary-validation primitives — `Validated` / `Rule` / `Validator` + `ValidatingTransformer` for validate-on-write |
...
- **`ValidatingTransformer(validator)`** (`vault-validation`) — validate-on-write transformer that wraps raw primitives into typed `Validated` domain objects at the boundary. A failed validation throws `IllegalArgumentException` and rolls the transaction back atomically.
